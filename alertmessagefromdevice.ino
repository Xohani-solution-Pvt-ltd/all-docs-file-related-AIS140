...
void setup() {
  ...
  Wire.begin();
  mpu.initialize();
  pinMode(panicSwitchPin, INPUT_PULLUP);
...
void sensorTask(void *pvParameters) {
  for (;;) {
    // Read sensor data
    readSensorData();
    int panicSwitchState = digitalRead(panicSwitchPin);  // Read the state of the panic switch
  
  if (panicSwitchState == LOW ) {  // If the panic switch is pressed
    Serial.println("Panic switch pressed!");  // Optional: Print a message to the serial monitor
    sendEmergencyData();
  } else {}
   
  
    delay(1000);  // Adjust t0he delay if needed
  }
}
...
void sendEmergencyData() {
  // Create a JSON object for emergency data
  DynamicJsonDocument emergencyDocument(1024);

  // Populate the emergency data
  emergencyDocument["StartCharacter"] = "$";
  emergencyDocument["PacketHeader"] = "EPB";
  emergencyDocument["PacketType"] = "EMR"; // Emergency Message
  emergencyDocument["IMEINumber"] = imei;
  emergencyDocument["PacketStatus"] = isConnected ? "NM" : "SP";
  emergencyDocument["Date"] = dateString + TimeString;
  emergencyDocument["GPSValidity"] = gps.location.isValid() ? "A" : "V";
  emergencyDocument["Latitude"] = gps.location.lat();
  emergencyDocument["LatitudeDirection"] = (gps.location.lat() >= 0) ? "N" : "S";
  emergencyDocument["Longitude"] = gps.location.lng();
  emergencyDocument["LongitudeDirection"] = (gps.location.lng() >= 0) ? "E" : "W";
  emergencyDocument["Altitude"] = gps.altitude.meters();
  emergencyDocument["Speed"] = gps.speed.kmph();
  emergencyDocument["Distance"] = "0"; // Replace with actual distance calculation
  emergencyDocument["Provider"] = "G"; // Fine GPS
  emergencyDocument["VehicleRegnNo"] = "OD00AB0000"; // Replace with actual registration number
  emergencyDocument["ReplyNumber"] = "+919109802925"; // Replace with actual mobile number
  emergencyDocument["Checksum"] = "DDE3220E";
  emergencyDocument["EndCharacter"] = "*";

  String emergencyData;
  serializeJson(emergencyDocument, emergencyData);

  // Use gprsSerial to send the emergency data to the server or mobile number
  if (isConnected) {
    gprsSerial.println("AT+HTTPPARA=\"URL\",\"http://65.0.85.221/api/v1/emergency\"");

     gprsSerial.println("AT+HTTPPARA=\"CONTENT\",\"application/json\"");
   delay(1000);
   //uShowSerialData();

   gprsSerial.println("AT+HTTPDATA=" + String(emergencyData.length()) + ",100000");
   Serial.println(emergencyData);
   delay(1000);
   //ShowSerialData();

   gprsSerial.println(emergencyData);
   delay(1000);
  // ShowSerialData();
//
   gprsSerial.println("AT+HTTPACTION=1");
 ...
 // ShowSerialData();
  ...
  // ShowSerialData();
 clearSerialBuffer(gprsSerial);
    // Additional HTTP setup commands if needed
  } else {
    // Send SMS to the specified mobile number
   sendSMS("+919109802925",emergencyData);
  }
